...
async function onPreActivatePureEmbeddableSessionFullPEF(message) {
    if (message.token) {
        ...
        localStorage.setItem("pureCloudEnvironment", iwscore.getConnectorConfig().auth.environment);
        ...
        try {
            ...
            window["gcUser"] = await pClient.usersApi.getUsersMe();
            if (iwscore.getConnectorConfig().eventType == "extended" || iwscore.getConnectorConfig().smartUi?.showConnectionLed) {
                var gcPresences = (await pClient.presenceApi.getPresencedefinitions({ "localeCode": "ALL", pageSize: 99 })).entities;
...
function onPreActivateSession(message) {
    ...
    if (message.activeInteractions) {
        message.activeInteractions.forEach(i => iwscore.addJSONObjectInMemory(i));
    }
...
function onPostSmsEventMarkDoneInbound(message) {
    iwscore.removeJSONObjectInMemory(message.ConnectionID);
}
//==============================================
//Whatsapp Section 
//==============================================
function onPreWhatsappEventEstablishedInbound(message) {
    iwscore.addJSONObjectInMemory(message, undefined);
}
function onPreWhatsappEventReleasedInbound(message) {
    window.dispatchEvent(new CustomEvent(enumCustomEventType.ConnectorInteractionReleased, { detail: message }));
}
function onPostWhatsappEventMarkDoneInbound(message) {
    iwscore.removeJSONObjectInMemory(message.ConnectionID);
}
//==============================================
//Instagram Section 
//==============================================
function onPreInstagramEventEstablishedInbound(message) {
    iwscore.addJSONObjectInMemory(message, undefined);
}
function onPreInstagramEventReleasedInbound(message) {
    window.dispatchEvent(new CustomEvent(enumCustomEventType.ConnectorInteractionReleased, { detail: message }));
}
function onPostInstagramEventMarkDoneInbound(message) {
    iwscore.removeJSONObjectInMemory(message.ConnectionID);
}
//==============================================
//Facebook Section 
...
function onPostFacebookEventMarkDoneInbound(message) {
    iwscore.removeJSONObjectInMemory(message.ConnectionID);
}
//==============================================
...
function onPreWebmessagingEventReleasedInbound(message) {
    window.dispatchEvent(new CustomEvent(enumCustomEventType.ConnectorInteractionReleased, { detail: message }));
}
function onPostWebmessagingEventMarkDoneInbound(message) {
    iwscore.removeJSONObjectInMemory(message.ConnectionID);
}
//==================================================================
//Events UserEvent
